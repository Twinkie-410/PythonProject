...
def join_salaries_field(df_):
    rub_exchange_rate = pd.read_csv("./data/currencies.csv")
    df_ = df_.fillna(0)
...
file_path = "data/vacancies_with_skills.csv"
df = pd.read_csv(file_path)
df = df.pipe(join_salaries_field)
df.to_csv("./data/vacancies_with_skills_middle_salary.csv", index=False)
